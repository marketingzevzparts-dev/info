...
В настройках KeyCRM (Налаштування → Webhooks / API) укажи URL:

```
https://<твой-домен>.up.railway.app/webhook/keycrm?secret=<WEBHOOK_SECRET>
```

и событие — создание карточки (card created / pipeline_card.created — конкретное название события зависит от версии KeyCRM).

**Важно:** после первого реального вебхука посмотри логи Railway (`console.log('Incoming KeyCRM webhook: ...')`) — там будет весь JSON, который прислал KeyCRM. Если названия полей (`pipeline_id`, `contact.phone` и т.п.) отличаются от того, что ожидает `extractCard()` в `index.js`, поправь эту функцию под реальный формат — так намного надёжнее, чем гадать по документации.
